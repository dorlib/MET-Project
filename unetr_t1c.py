...
SAVED_MODEL_PATH = os.path.join(BASE_DIR, "saved_models/brats_t1ce.pth")
...
def test_and_plot(model, loader, device, output_dir="test_results_t1ce"):
    ...
    for b, (x, y, img_names, mask_names) in enumerate(loader):
        ...
        for i in range(vols.shape[0]):
            ...
            plt.tight_layout()
            fig_filename = f"{base}_prediction_{suffix}_axes.png"
...
if __name__ == "__main__":
    ...
    model=build_unetr(); criterion=combined_loss
    #if os.path.exists(SAVED_MODEL_PATH):
    #   model.load_state_dict(torch.load(SAVED_MODEL_PATH,map_location=DEVICE))
    #   print("Loaded existing model, skipping training.")
    #else:
    # rain(model,train_ld,val_ld,criterion)
